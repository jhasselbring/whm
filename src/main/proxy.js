...
function onrequest(req, res) {
    console.log('@@@ onrequest', processURL(req.url));
    let server = this;
...
function onconnect(req, socket, head) {
    // req.url = '3.18.98.5:443';
    console.log('### onconnect', processURL(req.url));
    assert(
        !head || 0 == head.length,
...
function processURL(string) {
    if (string.includes(':')) {
        console.log('domain:PORT format');
        return
    }
    return string;
}

function ProcessHostPort(params) {
